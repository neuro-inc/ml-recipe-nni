...
class MnistNetwork(object):
    ...
    def __init__(self,
        ...
        self.images = tf.compat.v1.placeholder(tf.float32, [None, self.x_dim], name='input_x')
        self.labels = tf.compat.v1.placeholder(tf.float32, [None, self.y_dim], name='input_y')
    ...
    def build_network(self):
        ...
        with tf.compat.v1.name_scope('dropout'):
            h_fc1_drop = tf.nn.dropout(h_fc1, 1 - (self.keep_prob))
        ...
        with tf.compat.v1.name_scope('loss'):
            cross_entropy = tf.reduce_mean(
                input_tensor=tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(self.labels), logits=y_conv))
        ...
        with tf.compat.v1.name_scope('accuracy'):
            correct_prediction = tf.equal(
                tf.argmax(input=y_conv, axis=1), tf.argmax(input=self.labels, axis=1))
...
def conv2d(x_input, w_matrix):
    ...
    return tf.nn.conv2d(input=x_input, filters=w_matrix, strides=[1, 1, 1, 1], padding='SAME')


def max_pool(x_input, pool_size):
    """max_pool downsamples a feature map by 2X."""
    return tf.nn.max_pool2d(input=x_input, ksize=[1, pool_size, pool_size, 1],
                          strides=[1, pool_size, pool_size, 1], padding='SAME')


def avg_pool(x_input, pool_size):
    return tf.nn.avg_pool2d(input=x_input, ksize=[1, pool_size, pool_size, 1],
                          strides=[1, pool_size, pool_size, 1], padding='SAME')
